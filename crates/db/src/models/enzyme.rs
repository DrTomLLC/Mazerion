...
pub struct Enzyme {
    ...
    pub target_compounds: Option<String>,
...
impl Enzyme {
    pub fn validate(&self) -> Result<(), String> {
        ...
        let valid_types = ["pectinase", "amylase", "beta_glucanase", "protease", "other"];
        if !valid_types.contains(&self.enzyme_type.as_str()) {
            return Err(format!("Invalid enzyme type '{}', must be one of: {}",
                               self.enzyme_type, valid_types.join(", ")));
        }
