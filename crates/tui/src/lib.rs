...
use mazerion_core::{VALID_CATEGORIES, get_calculators_by_category};
use ratatui::{
    Terminal,
    ...
    io::Error: From<<B as ratatui::backend::Backend>::Error>,
{
    ...
    loop {
        terminal.draw(|f| {
            ...
            let category_tabs: Vec<Span> = state
                .categories
                .iter()
                .enumerate()
                .map(|(i, cat)| {
                ...
                .collect();
            let tabs = Paragraph::new(Line::from(category_tabs)).block(
                Block::default()
                    .borders(Borders::ALL)
                    .title("Categories [←/→]"),
            );
