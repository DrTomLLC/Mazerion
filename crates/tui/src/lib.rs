...
fn run_app<B: ratatui::backend::Backend>(terminal: &mut Terminal<B>) -> io::Result<()> where std::io::Error: std::convert::From<<B as ratatui::backend::Backend>::Error> {
    ...
    loop {
        terminal.draw(|f| {
            ...
            let title = Paragraph::new("🍯 Mazerion TUI - 39 Calculators | Press 'q' to quit")
            ...
            let items: Vec<ListItem> = calculators
                ...
                .map(|(i, calc_id)| {
                    ...
                    ListItem::new(vec![Line::from(vec![Span::raw(*calc_id)])]).style(style)
                })
                .collect();

            let list = List::new(items)
                .block(Block::default().title("Select Calculator (↑/↓, Enter to use)").borders(Borders::ALL));
