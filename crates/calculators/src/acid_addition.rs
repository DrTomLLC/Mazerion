...
impl Calculator for AcidAdditionCalculator {
    ...
    fn calculate(&self, input: CalcInput) -> Result<CalcResult> {
        ...
        let targ_ph: Decimal = target_ph
            .parse()
            .map_err(|_| Error::Parse("Invalid target_ph".into()))?;

        if targ_ph >= curr_ph {
            return Err(Error::Validation(
                "Target pH must be lower than current pH (acid lowers pH)".into(),
            ));
        }
        ...
        let acid_strength_factor = match acid_type {
            "tartaric" => Decimal::new(15, 1), // 1.5 g/L per 1.0 pH (strongest)
            "citric" => Decimal::new(17, 1),   // 1.7 g/L per 1.0 pH
            "malic" => Decimal::new(19, 1),    // 1.9 g/L per 1.0 pH
            "lactic" => Decimal::new(22, 1),   // 2.2 g/L per 1.0 pH (weakest)
            _ => Decimal::new(15, 1),          // default to tartaric
        };

        // Acid needed (g) = volume (L) × pH_diff × strength_factor
        let acid_needed_g = vol * ph_diff * acid_strength_factor;

        // Convert to teaspoons (approximately 5g per tsp for powdered acids)
        let acid_needed_tsp = acid_needed_g / Decimal::new(5, 0);
        ...
        if ph_diff > Decimal::new(5, 1) {
            result = result
                .with_warning("Large pH drop (>0.5) - add in stages and taste between additions");
        }

        if ph_diff > Decimal::ONE {
            result = result
                .with_warning("Very large pH drop (>1.0) - MUST add in multiple stages over days");
        }
        ...
        result =
            result.with_warning("Always add acid gradually - easy to add, impossible to remove");

        result = result
            .with_meta("acid_type", acid_name)
            .with_meta("acid_needed_g", format!("{:.2} g", acid_needed_g))
            .with_meta("acid_needed_tsp", format!("{:.2} tsp", acid_needed_tsp))
            .with_meta("current_ph", format!("{:.2}", curr_ph))
            .with_meta("target_ph", format!("{:.2}", targ_ph))
            .with_meta("ph_change", format!("{:.2}", ph_diff))
            .with_meta(
                "strength_factor",
                format!("{:.2} g/L per pH unit", acid_strength_factor),
            )
            .with_meta("volume_liters", format!("{:.2} L", vol));
