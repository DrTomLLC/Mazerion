...
impl Calculator for YeastStarterCalculator {
    ...
    fn calculate(&self, input: CalcInput) -> Result<CalcResult> {
        let cells_needed = input.get_param("cells_needed")
        ...
        let target_cells: Decimal = cells_needed.parse()
            .map_err(|_| Error::Parse("Invalid cells_needed".into()))?;
        let starting_cells: Decimal = cells_available.parse()
        ...
        if cells_to_grow <= Decimal::ZERO {
            return Err(Error::Validation("Already have enough cells, no starter needed".into()));
        }
        ...
        let mut result = CalcResult::new(Measurement::new(starter_volume * Decimal::from(1000), Unit::Milliliters));

        result = result
            .with_meta("starter_volume_L", format!("{:.1} L", starter_volume))
            .with_meta("dme_needed_g", format!("{:.0} g ({:.2} oz)", dme_needed, dme_needed / Decimal::new(2835, 2)))
        ...
        if final_cells < target_cells {
            result = result.with_warning("May need multiple starter steps to reach target cell count");
        }

        if starter_volume > Decimal::from(5) {
            result = result.with_warning("Large starter volume - consider stepped starter approach");
        }
