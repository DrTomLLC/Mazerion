...
impl Calculator for BacksweeteningCalculator {
    ...
    fn calculate(&self, input: CalcInput) -> Result<CalcResult> {
        ...
        let targ_sg: Decimal = target_sg.parse()
        ...
        if sg_diff > Decimal::new(80, 3) {
            result = result.with_warning("Very large gravity increase (>0.080) - MUST backsweeten in stages");
        }

        result = result.with_warning("CRITICAL: Stabilize with K-meta + K-sorbate BEFORE backsweetening");
