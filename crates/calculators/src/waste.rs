...
impl Calculator for WasteCalculator {
    ...
    fn calculate(&self, input: CalcInput) -> Result<CalcResult> {
        let initial_volume = input
            .get_param("initial_volume")
            .ok_or_else(|| Error::MissingInput("initial_volume required".into()))?;

        let num_rackings = input.get_param("num_rackings").unwrap_or("3"); // Default 3 rackings

        let vessel_type = input.get_param("vessel_type").unwrap_or("carboy"); // carboy, bucket, keg, barrel

        let process_type = input.get_param("process_type").unwrap_or("standard"); // standard, filtered, fined, none

        let vol_l: Decimal = initial_volume
            .parse()
            .map_err(|_| Error::Parse("Invalid volume".into()))?;

        let rackings: i32 = num_rackings
            .parse()
        ...
        let mut result = CalcResult::new(Measurement::new(final_volume_l, Unit::Grams))
            ...
            .with_meta(
                "total_loss",
                format!("{:.2} L ({:.1}%)", total_loss_l, loss_percent),
            )
        ...
        if rackings > 0 {
            result = result.with_meta(
                "loss_gross_lees",
                format!(
                    "{:.2} L ({:.1}%) - Primary fermentation sediment",
                    gross_lees_loss_l, gross_lees_percent
                ),
            );
        }
        ...
        if rackings > 0 {
            result = result.with_meta(
                "loss_transfers",
                format!(
                    "{:.2} L ({:.1}%) - Hose deadspace & spillage",
                    transfer_total_l, transfer_percent
                ),
            );
        }
        ...
        if clarification_loss_l > Decimal::ZERO {
            result = result.with_meta(
                ...
                format!(
                    "{:.2} L ({:.1}%) - {}",
                    clarification_loss_l, clarification_percent, process_type
                ),
            );
        }
        ...
        if rackings > 0 {
            result = result.with_meta(
                "racking_1",
                format!(
                    "Primary → Secondary: {:.2} L loss (gross lees + transfer)",
                    gross_lees_loss_l + transfer_loss_per_racking_l
                ),
            );
        }
        ...
        if loss_percent > Decimal::from(25) {
            result = result.with_warning(
                "⚠️ High loss rate (>25%) - consider fewer rackings or different vessel",
            );
        }
        ...
        if final_volume_l < vol_l * Decimal::new(6, 1) {
            result =
                result.with_warning("⚠️ Less than 60% recovery - process may be too aggressive");
        }
